...
--- (client/menu/server) You can attach callbacks to a number of compiler events with jit.attach. The callback can be called:* when a function has been compiled to bytecode ("bc");* when trace recording starts or stops ("trace");* as a trace is being recorded ("record");* or when a trace exits through a side exit ("texit").Set a callback with jit.attach(callback, "event") and clear the same callback with jit.attach(callback) 
...
--- () # Not a functionThis is NOT a function, it's a variable containing the version number of the LuaJIT core. Version `xx.yy.zz` is represented by the decimal number `xxyyzz`. In GMod this is `20004`. On x86-64 branch it's `20100`. 
