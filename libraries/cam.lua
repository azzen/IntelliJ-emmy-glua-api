...
--- (client) Switches the renderer back to the previous drawing mode from a 3D context.This function is an alias of [cam.End3D](https://wiki.facepunch.com/gmod/cam.End3D). 
...
--- (client) Tells the renderer to ignore the depth buffer and draw any upcoming operation "ontop" of everything that was drawn yet.This is identical to calling `render.DepthRange( 0, 0.01 )` for `true` and  `render.DepthRange( 0, 1 )` for `false`. See [render.DepthRange](https://wiki.facepunch.com/gmod/render.DepthRange). 
...
--- (client) Sets up a new 2D rendering context. Must be finished by [cam.End2D](https://wiki.facepunch.com/gmod/cam.End2D).This is almost always used with a render target from the [render](https://wiki.facepunch.com/gmod/render). To set its position use [render.SetViewPort](https://wiki.facepunch.com/gmod/render.SetViewPort) with a target already stored. 
--- [https://wiki.facepunch.com/gmod/cam.Start2D]
--- @return void
function cam.Start2D() end

--- (client) Sets up a new 3D rendering context. Must be finished by [cam.End3D](https://wiki.facepunch.com/gmod/cam.End3D).For more advanced settings such as an orthographic view, use [cam.Start](https://wiki.facepunch.com/gmod/cam.Start) instead. 
